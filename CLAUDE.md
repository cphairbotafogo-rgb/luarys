...
- **NFC-e (produto/PDV)** → tabela própria `nfce_emissoes` (migration `20260727_nfce_numero_atomico_persistencia.sql`). Numeração via RPC atômica `obter_proximo_numero_nfce` — **nunca** ler `proximo_numero` e atualizar em dois passos. Toda emissão grava registro ANTES de chamar o provedor (status `processando`) e atualiza depois — nota autorizada na SEFAZ sem registro local é falha fiscal grave (XML recuperável por 5 anos).
- **Brasil NFe é o único provedor** (Focus NFe removida em 30/07/2026). Dois tokens, e trocá-los é o erro mais comum: o **UserToken** da plataforma (`plataforma_nfse_config.token_brasilnfe`) só cadastra empresa; o **companyToken** por CNPJ (`saloes.config_fiscal.brasilnfe_company_token`) emite, consulta, cancela e sobe certificado.
- **A chave de acesso e os valores apurados vêm do XML, não do JSON.** A resposta da Brasil NFe não traz `Chave` nem `Valores.*` — ler de lá deixou 485 de 486 notas sem chave. Ver `lerXmlNFSe()` e **[FISCAL-REFERENCIA.md](FISCAL-REFERENCIA.md)**, que descreve a estrutura fiscal confirmada e os scripts de recuperação.
...
- **Resolvido:** Brasil NFe é o provedor único e emite no padrão nacional — confirmado pelos XML autorizados (`xmlns="http://www.sped.fazenda.gov.br/nfse"`, `verAplic: SefinNacional`), não por promessa comercial.
...
- **NFS-e:** rotas funcionais via Brasil NFe (`emitir`, `consultar`, `cancelar`, `upload-a1`, `webhook-brasilnfe`). **Emite no padrão nacional e o Rio aceita** — 486 notas autorizadas, conferidas nos XML. Pendente: try/catch por nota no lote (hoje uma exceção interrompe o restante) e emissão em produção, que nunca aconteceu.
- **NFC-e:** rota de emissão funcional (Brasil NFe) com numeração atômica e persistência em `nfce_emissoes`. Pendente: telas lerem de `nfce_emissoes`, e a NFC-e ainda não tem o equivalente ao link do portal nacional que a NFS-e tem — a consulta pública dela é da SEFAZ do estado, por QR Code.
...
- **Resumo diário por e-mail** (`/api/cron/resumo-diario`) — **PRONTO, MAS NÃO ATIVADO** por decisão do Ari (04/08/2026). A rota funciona, mas **não há entrada em `vercel.json`**: nada dispara sozinho. Envio automático é projeto futuro. Fechamento de ontem + agenda de hoje + CSV anexo; montagem em `src/lib/resumoDiario.ts`. Para ativar: (1) acrescentar o cron `0 4 * * *` em `vercel.json`, (2) configurar `RESEND_API_KEY` e `EMAIL_REMETENTE` (oficial: `Luarys <contato@luarys.com.br>`), (3) testar com disparo manual usando o `CRON_SECRET`.
- **E-mail oficial de saída:** `contato@luarys.com.br`. Precisa de domínio verificado no Resend antes de valer como remetente.
